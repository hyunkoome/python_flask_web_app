...
# 이미지 일람 화면에 테크 정보 표시
from apps.detector.forms import DetectorForm
...
# 왜냐하면, app.py에서 blueprint 등록시(register_blueprint) url_prefix를 세팅하지 않기 때문에
...
# 이미지 인지: 이미지 인지 함수, 인지 결과를 박스 치고, 결과 이미지 저장
def exec_detect(target_image_path):
    ...
    # 학습 완료 모델의 읽어 들이기
    model = torch.load(Path(current_app.root_path, "detector", "dets_models", "model.pt"))
...
# 이미지 인지: 이미지 인지 함수, 인지 결과(tag)를 DB에 저장
...
def delete_image(image_id):
    # user_images 테이블과 user_image_tags 테이블, 모두 삭제해야함
    # 따라서, try-except 사용해서, 레코드 삭제, 실패하면 롤백
    # 오류 발생시, flash 와 로그 출력
    try:
